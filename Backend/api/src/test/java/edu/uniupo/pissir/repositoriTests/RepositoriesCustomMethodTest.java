package edu.uniupo.pissir.repositoriTests;

import edu.uniupo.pissir.entity.*;
import edu.uniupo.pissir.repository.*;
...
public class RepositoriesCustomMethodTest {
    ...
    private AziendaAgricolaEntity aziendaAgricole;
    ...
    void setUp() {
        ...
        AziendaAgricolaEntity aziendaAgricolaEntity = new AziendaAgricolaEntity(
                asList(utenteEntity1, utenteEntity2), "azienda1", "this is a first azienda for my test implementation. have create this in beforeEach test method");


        aziendaAgricole = aziendaAgricolaRepository.save(aziendaAgricolaEntity);
        //Instanzia di Serre

        SerraEntity serraEntity1 = new SerraEntity(aziendaAgricole, "this is a first serra for my test implementation...");
        SerraEntity serraEntity2 = new SerraEntity(aziendaAgricole, "this is a second serra for my test implementation...");
        ...
        IrrigazionePianificatoreEntity pianificatore1 = new IrrigazionePianificatoreEntity(
                aziendaAgricole, serre.get(0), "Pianificatore1", LocalDate.of(2023, 4, 12), new ExecutionTimeEntity(LocalTime.of(12, 20, 10), LocalTime.of(16, 40, 25)));
        IrrigazionePianificatoreEntity pianificatore2 = new IrrigazionePianificatoreEntity(
                aziendaAgricole, serre.get(0), "Pianificatore2", LocalDate.of(2023, 8, 12), new ExecutionTimeEntity(LocalTime.of(6, 30, 40), LocalTime.of(10, 40, 30)));
    ...
    @Test
    void shouldFindAziendaAgricolaByIdUtenteIn() {

        List<UUID> idsUtente = List.of( utenti.get( 0 ).getIdUtente(), utenti.get( 1 ).getIdUtente() );

        Optional<AziendaAgricolaEntity> candidateToExpect = aziendaAgricolaRepository.findByUtenteEntitiesIdUtenteIn(idsUtente);
        ...
        assertThat(candidateToExpect).hasValueSatisfying(aziendaAgricola -> {
            ...
            assertThat(aziendaAgricola.getUtenteEntities()).extracting("idUtente").isEqualTo(idsUtente);
            assertThat(aziendaAgricola.getIdAziendaAgricola()).isEqualTo(aziendaAgricole.getIdAziendaAgricola());
    ...
    void shouldFindAziendaAgricolaByIdUtente () {
        ...
        Optional<AziendaAgricolaEntity> candidateToExpect = aziendaAgricolaRepository.findByUtenteEntitiesIdUtente( utenti.get( 1 ).getIdUtente() );
    ...
    void findAziendaAgricolaByHimNome() {
        String nome = aziendaAgricole.getNome();
        ...
        assertThat(candidateNomeToExcept).hasValueSatisfying(aziendaAgricola -> {
            assertThat(aziendaAgricola).isExactlyInstanceOf(AziendaAgricolaEntity.class);
            assertThat(aziendaAgricola.getNome()).isEqualTo(nome);
            assertThat(aziendaAgricola.getIdAziendaAgricola()).isEqualTo(aziendaAgricole.getIdAziendaAgricola());
            assertThat(aziendaAgricola.getDescrizione()).isNotBlank();
        });
    ...
    void shouldFindAziendaAgricolaByHimId() {
        UUID idAziendaAgricola = aziendaAgricole.getIdAziendaAgricola();
